...
export interface KpiCard05Props extends CommonFormattingProps {
    metrics?: MetricItem[];
    cols?: number;
...
const PeriodComparisonColoredCard: React.FC<{ item: MetricItem; valueFormatter?: (value: number | string) => string; valueSizeClass: string }> = ({
    item,
    valueFormatter,
    valueSizeClass,
}) => (
...
export const KpiCard05: React.FC<KpiCard05Props> = ({
    metrics = [],
    cols = 3,
    valueFormatter,
    ...
    return (
        <MetricCardGrid
            className={className}
            items={metrics}
            renderCard={(item) => <PeriodComparisonColoredCard item={item} valueFormatter={valueFormatter} valueSizeClass={valueSizeClass} />}
            cols={cols}
        />
    );
};
