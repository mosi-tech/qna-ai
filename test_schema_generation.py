#!/usr/bin/env python3
"""Test script for schema generation from docstrings"""
...
def extract_schema_from_docstring(func) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Failed to extract schema for {func.__name__}: {e}")
        return None
...
def test_schema_generation():
    """Test schema generation with actual functions"""
    try:
        # Import a function to test
        from analytics.indicators.technical import calculate_sma
        
        # Generate schema
        schema = extract_schema_from_docstring(calculate_sma)
        
        if schema:
            print("✅ Schema generated successfully for calculate_sma:")
            import json
            print(json.dumps(schema, indent=2))
            return True
        else:
            print("❌ No schema generated for calculate_sma")
            return False
...
if __name__ == "__main__":
    print("Testing Schema Generation from Docstrings")
    print("=" * 50)
