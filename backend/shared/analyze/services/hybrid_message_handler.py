...
from .financial_analyst_chat import FinancialAnalystChat, MessageIntent, ChatResponse
...
class HybridResponse:
    ...
    response_type: str  # "chat" or "analysis" or "analysis_queued"
    content: str
...
class HybridMessageHandler:
    ...
    def __init__(self, 
        ...
        self.financial_analyst = FinancialAnalystChat()
        
        # Track session state for context
    ...
    async def handle_message(self, 
        ...
        Routes between chat and analysis based on:
        1. Message intent classification
        2. Session context
        3. User confirmation of suggested analysis
        """
        try:
            ...
            # Get analyst chat response with intent classification
            start_time = time.time()
            chat_response = await self.financial_analyst.handle_message(
            ...
            logger.info(f"⏱️ Chat processing: {chat_duration:.3f}s, Intent: {chat_response.message_type.value}")
            
            # Route based on intent and response
            return await self._route_response(
    ...
    async def _route_response(self, 
        ...
        """Route the response based on chat analysis intent"""
