...
import os
from server import AnalysisLibraryMCP
...
class AnalysisLibraryClient:
    ...
    def __init__(self, db_path: str = None):
        ...
        self.library = AnalysisLibraryMCP(db_path)
    
    def save_analysis(self, question: str, function_name: str, docstring: str) -> dict:
        """Save a completed analysis to the library"""
        return self.library.save_analysis_direct(question, function_name, docstring)
    
    def search_similar(self, query: str, top_k: int = 5, similarity_threshold: float = 0.6) -> dict:
        """Search for similar analyses"""
        return self.library.search_similar_direct(query, top_k, similarity_threshold)

# Global instance for easy import
analysis_client = AnalysisLibraryClient()

# Convenience functions for direct import
def save_analysis(question: str, function_name: str, docstring: str) -> dict:
    """Save analysis - convenience function"""
    return analysis_client.save_analysis(question, function_name, docstring)

def search_similar_analyses(query: str, top_k: int = 5, similarity_threshold: float = 0.6) -> dict:
